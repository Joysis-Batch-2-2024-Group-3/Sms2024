...
public class Temp_View {
            ...
            try {
                switch (choice) {
                    ...
                    case 3 -> {
                        ...
                        int courseId = getValidCourseId(); // Input validation for Course ID
                    ...
                    case 5 -> {
                        ...
                        int newCourseId = getValidCourseId(); // Input validation for new Course ID
    ...
    private void updateSubjectField(){
        ...
        while (updating) {
            ...
            System.out.println("1. Update Subject ID");
            System.out.println("2. Update Name");
            System.out.println("3. Finish Update Subject");
            System.out.println("Enter to choose to update: ");

            try {
                int chooseToUpdateSubject = scan.nextInt();

                switch (chooseToUpdateSubject) {
                    case 1 -> {
                        System.out.println("Enter the new Subject ID:  ");
                        int newSubjectId = scan.nextInt();
                        subject.setSubject_id(newSubjectId);
                    }
                    case 2 -> {
                        System.out.println("Enter the new Subject Name: ");
                        String newSubjectName = scan.nextLine();
                        subject.setSubject_name(newSubjectName);
                    }
                    case 3 -> {
                        updating = false;
                        sub.updateSubject(subject);
                        //sub.displayAllSubject();
                    }
                    default -> System.out.println("Invalid choice to update. Please try again");
                }
            }catch (InputMismatchException e){
